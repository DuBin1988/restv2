...
namespace Com.Aote.ObjectTools
{
    ...
    /// 东进交换机类，
    /// </summary>
    public class AoPhone : CustomTypeHelper, IAsyncObject
    {
        #region 单例
        public static AoPhone instance;
        public static AoPhone GetInstance()
        {
            if (instance == null)
            {
                instance = new AoPhone();
            }
            return instance;
        }
        #endregion
      ...
      //登陆时间
        ...
        //传真状态
        private string faxHint;
        public string FaxHint
        {
            set
            {
                this.faxHint = value;
                OnPropertyChanged("FaxHint");
            }
            get
            {
                return this.faxHint;
        ...
        public void Playback()
        {
            ...
            jo.Add("aChannel", int.Parse(this.LineNum));
            jo.Add("aModule", 1);
            jo.Add("aBzType", "playback");
            jo.Add("aName", this.LoginName);
            jo.Add("bzExtra", this.RecordFile + ".wav");
            jo.Add("bzId", "123");
        ...
        {
            int lineNum = int.Parse(this.LineNum);
            JsonObject jo = new JsonObject();
            jo.Add("aId", this.GongHao);
            jo.Add("aChannel", int.Parse(this.LineNum));
            jo.Add("aModule", 1);
            jo.Add("aBzType", "callout");
            jo.Add("aName", this.LoginName);
            jo.Add("bzExtra", (String)GetPropertyValue("CallPhone"));
            jo.Add("bzId", "123");
        ...
        //监听
        public void ListenTo(string channelListened)
        {
            JsonObject jo = new JsonObject();
            jo.Add("aId", this.GongHao);
            jo.Add("aChannel", int.Parse(this.LineNum));
            jo.Add("aModule", 1);
            jo.Add("aBzType", "listen");
            jo.Add("aName", this.LoginName);
            jo.Add("bzExtra", channelListened);
            //dsp module id
            jo.Add("bzId", "1");
        ...
        public void SetFree()
        {
            ...
            client.UploadStringCompleted += (o, a) =>
            {
                if (a.Error != null)
                {
                    MessageBox.Show("置闲操作失败!");
                }
                else
                {
                    JsonObject js = (JsonObject)JsonObject.Parse(a.Result);
                    if (js["aState"] != "" && js["aState"] == "空闲")
                        this.CallNumber = "";
                }
            };
            client.Headers["Content-Type"] = "text/plain";
