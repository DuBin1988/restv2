      ...
      /**  public static string ToMD5(object obj)
        {
            ...
            result = MD5.GetMd5String(obj.ToString());
            return result;
        }**/
        ...
        //把日期转化到月底
        public static DateTime ToMonthEnd(object obj)
        {
            ...
            throw new Exception("只有日期型可以转成日期");
        }
        ...
        //长安PDA日均用气量转化成列表，供曲线图显示
        public static BaseObjectList ToList2(object list)
        {
            //只有枚举对象才可以转换成通用对象
            if (!(list is IEnumerable))
            {
        ...
        public static string FormatToMoney(decimal obj)
        {
            if (obj == null)
            {
                return "0.00";
        ...
        #region AddM_ 往ObjectList里添加一或多项
        public static void AddM_(object list, object list2)
        {
            if (!(list is BaseObjectList) || !(list2 is IList))
                throw new Exception("只能把IList放进BasicObjectList中");
            BaseObjectList l = list as BaseObjectList;
            IList l2 = list2 as IList;
            foreach (GeneralObject item in l2)
            {
                l.Add(item);
            }
        }
