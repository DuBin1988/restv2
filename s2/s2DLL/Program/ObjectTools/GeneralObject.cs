...
namespace Com.Aote.ObjectTools
{
    ...
    public class GeneralObject : CustomTypeHelper, IAsyncObject, ILoadable, IFromJson
    {
        ...
        #region CustomePath 工作流路径字段，如果存在改变去请求的url。

        public static readonly DependencyProperty CustomePathProperty =
            DependencyProperty.Register("CustomePath", typeof(string), typeof(GeneralObject),
            new PropertyMetadata(new PropertyChangedCallback(OnCustomePathChanged)));

        private static void OnCustomePathChanged(DependencyObject dp, DependencyPropertyChangedEventArgs args)
        {

        }

        public string CustomePath
        {
            get { return (string)GetValue(CustomePathProperty); }
            set { SetValue(CustomePathProperty, value); }
        }
        #endregion
    }
}
