...
namespace Com.Aote.ObjectTools
{
    ...
    public class BJICCard : CustomTypeHelper, IAsyncObject
    {
        ...
        public void SellGas()
        {
            ...
            //如果是金额表，要先传入金额
            GetEvent("SetMoney").SetMoney(Money);
            //如果是金额表，要先传入单价
            GetEvent("SetPrice").SetPrice(JEPrice);
            //如果是金额表，上次金额
            GetEvent("SetScMoney").SetScMoney(ScBuyMoney);
            //如果是金额表，上上次金额
            GetEvent("SetSscMoney").SetSscMoney(SscBuyMoney);
            //设置总购气量
            GetEvent("SetHTZongBuy").SetHTZongBuy(Memetergasnums);
            //设置表号
            GetEvent("SetMeterID").SetMeterID(MeterID);
            Log.Debug("SellGas start:  Factory:" + Factory + "|CardId:" + CardId + "|Gas:" + Gas + "|BuyTimes:" + BuyTimes + "|Price:" + Price + "|ScBuyGas:" + ScBuyGas + "|SscBuyGas:" + SscBuyGas);
            int sellRen = GetEvent("SellGas").SellGas(Factory, CardId, Gas, BuyTimes, Price, ScBuyGas, SscBuyGas);
            Log.Debug("Sellgas return:" + sellRen);
            //售气成功返回true,失败false;
            if (sellRen==0)
            {
                State = State.End;
        ...
        public void ReInitCard()
        {
            ...
            GetEvent("SetMeterID").SetMeterID(MeterID);
            //发卡
            Log.Debug("Init start:  Factory:" + Factory + "|CardId:" + CardId + "|Gas:" + Gas + "|BuyTimes:" + BuyTimes + "|Price:" + Price);
            int initRen = GetEvent("Init").Init(Factory, CardId, Gas, BuyTimes, Price);
            Log.Debug("Init return:" + initRen);
            if (initRen == 0)
            {
                State = State.End;
            }
        ...
        //总购气量
        private double memetergasnums = 0;
        public double Memetergasnums
        {
            get { return this.memetergasnums; }
            set { this.memetergasnums = value; }
        }
        //上次购气金额
        private double scbuymoney = 0;
        public double ScBuyMoney
        {
            get { return this.scbuymoney; }
            set { this.scbuymoney = value; }
        }
        //上上次购气金额
        private double sscbuymoney = 0;
        public double SscBuyMoney
        {
            get { return this.sscbuymoney; }
            set { this.sscbuymoney = value; }
        }
        //卡内气量
        private double gas;
        public double Gas
